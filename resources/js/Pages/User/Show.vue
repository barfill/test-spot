<template>
    <Breadcrumbs v-if="$page.props.user.type === 'admin'"
        :breadcrumbs="[
            { label: translations.users, link: route('user.index', { locale }) },
            { label: translations.edit_user, link: '#' }
    ]"
    ...
    <Breadcrumbs v-if="$page.props.user.type !== 'admin'"
        :breadcrumbs="[
            { label: translations.edit_profile, link: '#' }
    ]"
        :translations="translations"
        :locale="locale"
    />
    ...
    >
        <div class="flex flex-col">
            <div>
                <label for="email" class="label label-enabled">{{ translations.email }}</label>
            ...
            <div>
                <label for="password" class="label label-enabled">{{ translations.password }}</label>
            ...
            <div v-if="$page.props.user.type === 'admin'">
...
<script setup>
    ...
    const form = useForm({
        ...
        password: '',
        type: props.editedUser.type,
        title: props.editedUser.title,
        first_name: props.editedUser.first_name,
        last_name: props.editedUser.last_name,
    });

    const update = () => form.post(route('user.update', { locale: props.locale, user: props.editedUser }));
</script>
